...
\[
J = \frac{1}{2}\sum_{n=0}^{\infty}\left(20x_1[n]^2 + x_2[n]^2 +4u[n]^2 \right) 
\]
This is just a standard quadratic optimization of the form $J = \frac{1}{2}\sum\limits_{n=0}^{\infty}\left(\mathbf{x}[n]^T\mathbf{Qx}[n] + \mathbf{u}[n]^T\mathbf{Ru}[n] \right) $ where $\mathbf{Q} = \begin{bmatrix}20 & 0\\0 & 1\end{bmatrix}$ and $\mathbf{R} = 4$. 
...
\[\mathbf{K} = \begin{bmatrix} 1.0821 & 0.3319\end{bmatrix}\]
And our system poles are:
\begin{align*}
z_1 &= 0.8314 \\ z_2 &= 0.8734
\end{align*}
...
\begin{figure}[h]
\centering
\includegraphics[width = 0.7\linewidth]{sim_results}
